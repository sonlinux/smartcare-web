#!/bin/bash
echo "Use this to build a production image of the smartcare-client-uwsgi"
...
then
    ...
    cd ..
	git archive --format=tar.gz $VERSION django_project > deployment/docker/smartcare-client.tar.gz
    cd -
    ...
    echo "ADD smartcare-client.tar.gz /home/web" >> docker/Dockerfile-prod
    echo "CMD [\"uwsgi\", \"--ini\", \"/uwsgi.conf\"]" >> docker/Dockerfile-prod
    # Now build the image and tag it
    cd docker
	docker build -t edited/smartcare-uwsgi:latest -f Dockerfile-prod .
    docker tag edited/smartcare-uwsgi:latest edited/smartcare-client-uwsgi:$VERSION
    docker push edited/smartcare-uwsgi:latest
    docker push edited/smartcare-uwsgi:$VERSION
    cd -
	# Clean up
